  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (is429 && attempt < retries) {
        // Wait 15s on first retry, 30s on second
...
export async function POST(req: Request) {
  try {
    ...
    return Response.json(
      {
        error: is429
          ? "Too many requests — the free AI quota is temporarily exhausted. Please wait 30 seconds and try again."
